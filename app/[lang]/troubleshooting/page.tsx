...
const tocByLang: Record<Locale, TocItem[]> = {
  en: [
    ...
    {
      id: "led-guide",
      label: "LED Status Guide",
      children: [
        { id: "left-side", label: "Left Side (Device Side)" },
        { id: "right-side", label: "Right Side (Host Side)" },
      ],
    },
    {
      ...
      children: [
        { id: "both-fast", label: "Both LEDs Fast Blinking" },
        { id: "left-fast", label: "Left Side Fast Blink" },
        { id: "right-fast", label: "Right Side Fast Blink" },
  ...
  cn: [
    { id: "prerequisites", label: "前提条件" },
    {
      ...
      label: "LED 状态指南",
      children: [
        { id: "left-side", label: "左侧（设备侧）" },
        { id: "right-side", label: "右侧（主机侧）" },
      ],
    },
    {
      id: "troubleshooting-steps",
      label: "故障排除步骤",
      children: [
        { id: "both-fast", label: "两侧 LED 快闪" },
        { id: "left-fast", label: "左侧快闪" },
        { id: "right-fast", label: "右侧快闪" },
              ...
              <nav className="mt-4 space-y-3 text-sm">
                {toc.map((item) => (
                  <div key={item.id} className="space-y-2">
                    <Link
                      href={`/${lang}/troubleshooting#${item.id}`}
                    ...
                    {item.children?.length ? (
                      <ul className="space-y-1 border-l border-border/60 pl-3 text-xs text-muted-foreground">
                        {item.children.map((child) => (
                          <li key={child.id}>
                            <Link
                              href={`/${lang}/troubleshooting#${child.id}`}
